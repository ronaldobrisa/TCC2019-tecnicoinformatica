...
function delRecord(id) {   
     ...
     if (teste==false) {
     	       window.location = "listfuncionarios.php";
            } 
     else window.location = "dodelfuncionarios.php?cod="+id;
 }
 function altnewindow(id) {
	  openwindow = window.open("altfuncionarios.php?cod="+id, "mywindow","location=1, status=1, scrollbars=1, width=600, height=700");
	  testwindow.moveTo(0,0);
		} 
function insnewindow() {
	  openwindow = window.open("insfuncionarios.php", "mywindow","location=1, status=1, scrollbars=1, width=600, height=700");
	  testwindow.moveTo(0,0)
		}
...
while ($linha = mysqli_fetch_array($query)) {
 	 ...
 	 echo "<td style='text-align:center'>$linha[id_funcionario]</td>";
 	 echo "<td>$linha[nome_funcionario]</td>";
 	 echo "<td>$linha[login_cpf]</td>";
 	 echo "<td>$linha[data_cadastro]</td>";
 	 echo "<td>$linha[cpf_funcionario]</td>";	 
 	 //echo "<td style='text-align:center'>";
 	 
 	 //echo "<td style='text-align:center'>";
 	 echo "<a href='javascript:delRecord(".$linha['id_funcionario'].");'>X</a>";
 	 echo "</td>";
 	 echo "<td style='text-align:center'>";
 	 echo "<a href='javascript:altnewindow(".$linha['id_funcionario'].");'>";
